...
public:
	LineDescriptor();
	LineDescriptor(unsigned int numOfBand, unsigned int widthOfBand, EDLineParam lineParam);
	~LineDescriptor();
...
public:
	...
	int Run(float scaleX, float scaleY, boundingbox_t bbox,
		image_int8u_p image, ScaleLineSet & keyLines, int gs_kx = 0, int gs_ky = 0, float sigma_scale = 0.6f);
...
LineDescriptor::LineDescriptor(unsigned int numOfBand, unsigned int widthOfBand, EDLineParam lineParam)
{
	edLineVec_.resize(ONE);
	edLineVec_[ZERO] = new EDLineDetector(lineParam);
...
static image_int8u_p gaussian_sampler_byte_bbox(image_int8u_p in, boundingbox_t bbox, pixel_float_t scale,
	float sigma_scale, int gs_kx = 0, int gs_ky = 0)
{
	...
	if (gs_ky > 1) {
		_h = (unsigned int)ceil((gs_ky - 1) * 0.5);
	}
...
int LineDescriptor::Run(float scaleX, float scaleY, boundingbox_t bbox,
	image_int8u_p image, ScaleLineSet & keyLines, int gs_kx, int gs_ky, float sigma_scale)
{
	...
	int gs_kx = 0, int gs_ky = 0, float sigma_scale = 0.6f)
{
	...
	if (lineDesc.Run(scaleX, scaleY, bbox, _src, lineVec, gs_kx, gs_ky, sigma_scale))
		return 1;
...
{
	if (src == NULL) {
		cerr << "src cannot be null!" << endl;
		return 1;
	}

	if (scaleX * bbox.width < 10 || scaleY * bbox.height < 10) {
		cerr << "box size(width of height) must great than 10" << endl;
		return 1;
	}
...
double* EdgeDrawingLineDetectorWrapper(int& n_out, unsigned char *src, int w, int h,
		float scaleX, float scaleY, short gradThres, short anchorThres, short scanInterval, int minLineLen,
	float lineFitErrThres, int gs_kx, int gs_ky, float gs_sigma){
	...
	n_out = lines.size();
	double* lineArr = new double[n_out * 4];
    for (int i = 0; i < lines.size(); i++) {
        lineArr[i * 4] = lines[i].startx;
        lineArr[i * 4 + 1] = lines[i].starty;
        lineArr[i * 4 + 2] = lines[i].endx;
        lineArr[i * 4 + 3] = lines[i].endy;
    }
	return lineArr;
}

void free_lines(double* rtv) {
    if (rtv != NULL) {
        free((void*)rtv);
    }
}